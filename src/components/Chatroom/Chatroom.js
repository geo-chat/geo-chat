...
class Chatroom extends Component {
	...
	async componentDidMount() {
		...
		const socket = io('http://192.168.0.22:7777/chat', { secure: true });
		// const socket = io("http://192.168.254.58:7777/chat", { secure: true });
						...
						{this.state.users.map((user, index) => {
							return (
								<div className="borderBoxChatRoom" key={index}>
									{user.img !== '' ? (
										<img className="imageInSideChat" alt="user img" src={user.img} />
									) : (
										<img
...
export default connect(mapStateToProps, { addToRoom, leaveRoom, getRooms, getUser })(Chatroom);
